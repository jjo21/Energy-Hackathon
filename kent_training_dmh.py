...
dch['DM-H Price'] = tavp['Ancillary Price - DM-H - GB (£/MW/h)'].copy()
dch['Trend'] = np.where(dch['DM-H Price'] - dch['DM-H Price'].shift(1) > 0, 1, 0)
...
plt.title(f'DM-H Trend Feature Importances RMSE: {rmse:.2f}')
plt.savefig('kent_result_plots/dmh_trend_feature_importances.png')
...
plt.title('DM-H Price Feature Importances')
plt.savefig('kent_result_plots/dmh_price_feature_importances.png')

plt.figure(figsize=(10, 4))
plt.plot(y_test.values, label='Actual')
plt.plot(y_pred, label='Predicted')
plt.legend()
plt.title(f'Actual vs Predicted DM-H Prices RMSE: {rmse:.2f} R²: {r2:.2f}')
plt.savefig('kent_result_plots/dmh_price_predictions.png')